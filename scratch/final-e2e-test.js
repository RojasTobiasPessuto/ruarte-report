...
// SIMULACIÓN DE 3 FILAS (Cuota 1, 2 y 3)
function getRow(nroCuota, revenue, monto) {
  ...
  row[13] = nroCuota === 1 ? revenue : 0; // N (Solo Cuota 1 tiene Revenue)
  row[14] = monto; // O (Cash)
...
async function runTest() {
  ...
  console.log('Obteniendo última fila...');
  const metaRes = await fetch(`https://services.googleapis.com/v4/spreadsheets/${sheetId}/values/${encodeURIComponent('CRM Agendas!B:B')}?access_token=${token}`);
  const metaData = await metaRes.json();
  const nextRow = (metaData.values ? metaData.values.length : 0) + 1;

  console.log(`Insertando 3 filas a partir de la ${nextRow}...`);

  for (let i = 0; i < rowsToInsert.length; i++) {
    const currentRow = nextRow + i;
    const rowData = rowsToInsert[i];
    
    const blocks = [
      { range: `B${currentRow}:P${currentRow}`, values: [rowData.slice(1, 16)] },
      { range: `V${currentRow}:Z${currentRow}`, values: [rowData.slice(21, 26)] },
      { range: `AB${currentRow}:AB${currentRow}`, values: [[rowData[27]]] }
    ];

    for (const block of blocks) {
      await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/${encodeURIComponent('CRM Agendas!' + block.range)}?valueInputOption=USER_ENTERED&access_token=${token}`, {
        method: 'PUT',
  ...
  console.log('✅ TEST E2E FINALIZADO CON ÉXITO.');
}
