...
// DATOS REALES DEL LEAD CREADO
const newRow = new Array(27).fill('');
newRow[0] = 'Cliente';
newRow[1] = 'real_test_claude@ruartereports.com';
newRow[2] = 'e1bQXBOxudX2uKUs9710'; // Contact ID Real
newRow[3] = 'Claude Real E2E Test';
newRow[4] = 'AR';
...
newRow[23] = 'Sc7Q9vF8QYBUOmo7Xa3l'; // Opp ID Real

async function run() {
  try {
    console.log('Sincronizando lead real con Google Sheets...');
    ...
    const res = await fetch(url, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ values: [newRow] }) });
    if (res.ok) {
      console.log('✅ ÉXITO: El lead "Claude Real E2E Test" ya debe estar en tu Sheet.');
    } else {
