...
async function writeDirect() {
  console.log('Intentando escritura directa en Fila 1160...');
  const token = await getAccessToken();
  const url = `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/${encodeURIComponent('CRM Agendas!A1160:D1160')}?valueInputOption=USER_ENTERED`;
  
  const res = await fetch(url, {
    method: 'PUT',
    headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ values: [['TEST', 'TEST@CLAUDE.COM', 'TEST_ID', 'CLAUDE TEST DIRECT']] })
  });
